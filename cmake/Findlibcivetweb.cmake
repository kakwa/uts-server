if(NOT LIBCIVETWEB_LIBRARIES)
  find_path(LIBCIVETWEB_INCLUDE_DIR civetweb.h ${_LIBCIVETWEB_PATHS}
            PATH_SUFFIXES include include/civetweb/)
endif()

if(NOT LIBCIVETWE[DB_LIBRARIES)
  if(LIBCIVETWEB_USE_STATIC_LIBS)
    find_library(
      LIBCIVETWEB_LIBRARIES
      NAMES libcivetweb.a ${_LIBCIVETWEB_PATHS}
      PATH_SUFFIXES lib)
  else(LIBCIVETWEB_USE_STATIC_LIBS)
    find_library(
      LIBCIVETWEB_LIBRARIES
      NAMES civetweb ${_LIBCIVETWEB_PATHS}
      PATH_SUFFIXES lib)
  endif(LIBCIVETWEB_USE_STATIC_LIBS)
endif()
